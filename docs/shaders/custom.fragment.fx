...
void main() {
    ...
    bool is_even = int(row) % 4 <= 1;
    if (is_even) {
        FragColor = vec4(((FragColor.r + 0.5) * texture(image, red_offset).r), ((FragColor.g + 0.5) * texture(image, tex_coord).g), ((FragColor.b + 0.5) * texture(image, blue_offset).b), FragColor.a) + 0.1;
    } else {
        FragColor = vec4((FragColor.r * texture(image, red_offset).r), (FragColor.g * texture(image, tex_coord).g), (FragColor.b * texture(image, blue_offset).b), FragColor.a);
    }
}
